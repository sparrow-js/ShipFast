...
import { obx, computed, autorun, makeObservable, runInAction, wrapWithEventSwitch, action } from '@/engine/editor-core';
import {
  isDOMText,
  isJSExpression,
  NodeSchema,
...
import { compatStage } from '@/engine/utils';
import { SettingTopEntry } from '@/engine/designer';
import { Node as ShellNode } from '@/engine/shell';
...
export class Node<Schema extends NodeSchema = NodeSchema> {
  ...
  @computed get dataMeta(): ComponentMeta {
    return this.document.getDataMeta(this.componentName);
  }
  ...
  setDataValue(path: string, value: any) {
    this.getData(path, true)!.setValue(value);
    this.getProp(`data.${path}`, true)!.setValue(value);
  }
