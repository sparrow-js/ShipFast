...
import { Title } from '@/engine/editor-core';
import { IEditor, TitleContent } from '@/engine/types';
...
export class Field extends Component<FieldProps> {
  ...
  constructor(props: any) {
    ...
    this.clickHandlerAi = this.clickHandlerAi.bind(this);
  }
  ...
  async clickHandlerAi(event?: MouseEvent) {
    ...
    try {
      const uri = "/api/get-ai-content";
      const params = {
        prompt: page.getData().prompt
      };
      ...
      if (resp.ok) {
        ...
        if (res.data) {
          children.props.onChange(res.data.content);
          this.setState({
            aiLoading: false,
          });
          return;
  ...
  render() {
    ...
    const { className, children, meta, title, valueState, name: propName, tip, supportAi } = this.props;
      ...
      >
        {
          display !== 'plain' && (
            <div className="lc-field-head" onClick={isAccordion ? this.toggleExpand : undefined}>
              ...
              {supportAi && (
                <span onClick={this.clickHandlerAi} className='flex justify-center items-center text-indigo-600 hover:text-indigo-400 cursor-pointer'>
