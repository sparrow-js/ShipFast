...
import { Title, observer, Editor, obx, globalContext, engineConfig, makeObservable } from '@/engine/editor-core';
import { Node, isSettingField, SettingField, Designer } from '@/engine/designer';
import classNames from 'classnames';
import { SettingsMain } from './main';
import { SettingsPane } from './settings-pane';
import { StageBox } from '../stage-box';
import { SkeletonContext } from '../../context';
import { createIcon } from '@/engine/utils';
...
export class SettingsPrimaryPane extends Component<{ editor: Editor; config: any }, { shouldIgnoreRoot: boolean }> {
  ...
  componentDidMount() {
    ...
    globalContext.get('editor').on('designer.selection.change', () => {
      ...
      const node = settings?.first
      if (node) {
        skeleton.rightArea.show();
      }
      this.main.hideSubSettingView();
    });
  }
  ...
  render() {
    ...
    return (
      <div className={className}>
        {/* { this.renderBreadcrumb() } */}
        <div className="max-w-full flex items-center justify-between bg-white z-10 py-2 pr-1 md:pr-3 md:rounded-t-lg pl-2 md:pl-4 border-b h-[53px]">
          <div className="relative flex items-center z-5">
            {
              this.main.visbleSubSettingView ? (
                <p className="text-gray-900 ml-0 mb-0 py-2 transition-all duration-300 flex justify-center items-center">
                  <span 
                    className='p-2 rounded-sm	hover:bg-slate-100 mr-1'
                    onClick={() => {
                      this.main.hideSubSettingView();
                    }}
                  >
                ...
                <p className="font-bold text-gray-900 ml-0 mb-0 py-2 transition-all duration-300">
                  {settings.first.componentName}
                </p>
              )
            }
          ...
          <div>
            ...
            <button onClick={() => {
              this.settingDone();
            }} type="button" className="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 border border-input bg-background shadow-sm hover:bg-accent hover:text-accent-foreground px-2 py-1">
              Done
            </button>
            </div>
          </div>
        ...
        <Tab
          ...
          style={{
            display: this.main.visbleSubSettingView ? 'none' : 'block'
          }}
